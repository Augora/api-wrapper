...
export function promiseToObservable(fromPromise : IFromPromise, promise : Promise<any>) : Observable<any> {
...
export function getAsObservable(axiosGet : IAxiosGet, promiseToObservableFunc: IFromPromise, url: string, config: object) : Observable<any> {
  const axiosPromise = axiosGet(url, config);
  return promiseToObservableFunc(axiosPromise);
}

export function getAsObservableFactory(axiosGet : IAxiosGet, promiseToObservableFunc : IFromPromise) {
  return getAsObservable.bind(undefined, axiosGet, promiseToObservableFunc);
}

export const providedGetAsObservable = getAsObservableFactory(axios.get, localProvidedPromiseToObservable);
